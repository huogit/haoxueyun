<template>
  <div class="cart py-container">
    <div class="checkout-tit" style="width: 1050px; margin: 0 auto; padding: 10px 0;">
      <h4 class="fl tit-txt">
        <span class="success-info">支付申请提交成功，请您及时付款！订单号：{{ payObj.out_trade_no }}</span>
      </h4>
      <span class="fr">
        <em class="sui-lead">应付金额：</em><em class="orange money">￥{{ payObj.total_fee }}</em>
      </span>
      <div class="clearfix"/>
    </div>
...
export default {
  async asyncData(page) {
    ...
    return {
      payObj: response.data.native // 包含支付二维码地址
    }
  },
  data() {
    return {
    ...
    queryPayStatus() {
      orderApi.queryPayStatus(this.payObj.out_trade_no).then((response) => {
        ...
        if (response.success) {
          ...
          // 三秒后跳转到课程详情页面观看视频
          setTimeout(() => {
            this.$route.push({ path: '/course/' + this.payObj.course_id })
          }, 3000)
        }
      })
    }
