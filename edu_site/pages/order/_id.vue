<template>
  <div class="Page Confirm">
    ...
    <div class="Finish">
      <div class="check fr">
        <el-checkbox v-model="agree">我已阅读并同意<a href="javascript:" target="_blank">《购买协议》</a></el-checkbox>
      </div>
    </div>
    <div class="clear"/>
    <div class="Main fl">
...
export default {
  ...
  methods: {
    fetchData() {
      orderApi.getById(this.order.id).then((response) => {
        this.order = response.data.item
      })
    },
    toPay() {
      if (this.agree) {
        this.$route.push({ path: '/pay/' + this.order.orderNo })
      }
    }
  }
}
