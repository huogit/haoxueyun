    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-button
            v-if="(scope.row.level == 1 || scope.row.level == 2) && hasPerm('permission.create')"
            plain
            icon="el-icon-edit"
            size="mini"
          ...
          <el-button
            v-if="scope.row.level == 3 && hasPerm('permission.create') "
            plain
            icon="el-icon-edit"
            size="mini"
    ...
    <el-dialog :visible.sync="dialogFormVisible" :title="dialogFormValue">
      <el-form ref="menu" :model="menu" :rules="menuValidateRules" label-width="120px">
        ...
        <el-form-item label="路由名字">
          <el-input v-model="permission.routeName"/>
        </el-form-item>
...
export default {
  ...
  watch: {
    filterText(filterText) {
      this.$refs.subjectTree.filter(filterText)
    }
  },
  ...
  // },
  mounted() {
    if (this.hasPerm('permission.list')) {
      this.fetchNodeList()
    } else {
      this.$message({
        type: 'warning',
  ...
  methods: {
    fetchNodeList() {
      permissionApi.getNestedTreeList().then(response => {
        console.log(response)
        if (response.success === true) {
          // this.menuList = response.data.items
          var list = []
          this.getJsonToList(list, response.data.items)
          this.menuList = list
    ...
    getJsonToList(list, jsonList) {
      var parseJson = []
      parseJson = JSON.parse(JSON.stringify(jsonList))
      // 遍历这个集合对象，获取key的值
      for (var i = 0; i < parseJson.length; i++) {
        list.push(parseJson[i])
      }
      console.log(list)
    },
    ...
    remove(data) {
      ...
      this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
        ...
        return permissionApi.removeById(data.id)
      }).then(() => {
      ...
      this.$refs.permission.validate(valid => {
        if (valid) {
          if (this.permission.id) {
            ...
            permissionApi.saveLevelOne(this.permission).then(response => {
    ...
    appendLevelOne() {
      permissionApi.saveLevelOne(this.menu)
        .then(response => {
    ...
    update(obj) {
      debugger
      permissionApi.update(obj).then(response => {
        this.dialogFormVisible = false
        this.$message({
          type: 'success',
          message: '修改成功'
